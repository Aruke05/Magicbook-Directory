...
export default function TemplatesPage() {
  ...
  const templates = listTemplates().map((item) => ({
    ...
    fieldCount: item.fields.length, ruleCount: item.ruleCount, updatedAt: formatDate(item.updatedAt), categoryId: item.categoryId, categoryName: item.categoryId ? categoryNames.get(item.categoryId) ?? "未分类" : "未分类",
  }))
