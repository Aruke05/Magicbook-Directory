...
export default function HistoryPage() {
  const items = db.select().from(promptGenerations).orderBy(desc(promptGenerations.createdAt)).limit(100).all().map((item) => ({ id: item.id, templateId: item.templateId, templateName: item.templateName, matchedRuleName: item.matchedRuleName, source: item.source, inputData: safeJsonParse<Record<string, unknown>>(item.inputData, {}), content: item.generatedContent, createdAt: formatDate(item.createdAt) }))
