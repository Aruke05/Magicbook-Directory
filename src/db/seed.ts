...
const strategyFields: PromptField[] = [
  field("sourceService", "参考服务", "select", 0, { required: true, group: "对接任务", options: [
    "nexapay", "lakpay", "devapay", "nine", "poppay", "mcb", "6g", "vns", "nupa", "tata",
  ...
  field("branch", "目标分支", "mapping", 2, {
    ...
    mapping: {
      ...
      rules: [
        ...
        { sourceValues: ["mcb", "6g", "vns", "nupa", "tata"], output: "prod-pkr" },
      ],
    },
  }),
  field("integrationInfo", "对接信息", "markdown", 3, {
    required: true,
    group: "对接资料",
...
要求：
1. df_place_order、df_notify代付下单一定要仔细查看成功和失败规则，并且一定要着重检查这两项的正确性；如果不确定就留空。
...
8. mcbpay服务的接口jazz和ep虽然可以配置成一个通道，但这里硬性要求分开通道，名称上要有区别，参考其他上游。
9. 注意SKT，也就是收银台下单和非收银台下单两种代收下单接口，一定会有一个直连下单接口。
